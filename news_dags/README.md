...
| `news_dags/`       | A Python directory that contains code for your Dagster repository                 |
| `news_dags_tests/` | A Python directory that contains tests for `news_dags`                      |
...
pyenv virtualenv $PYTHON_VERSION news_dags
pyenv activate news_dags
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name news_dags python=PYTHON_VERSION
conda activate news_dags
```
...
Tests can be found in `news_dags_tests` and are run with the following command:

```bash
pytest news_dags_tests
```

As you create Dagster ops and graphs, add tests in `news_dags_tests/` to check that your
